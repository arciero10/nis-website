...
export default function Footer(){return <footer className="footer">
  <div className="shell footer-grid">
    ...
    <div className="footer-column"><h2>Informazioni</h2><Link href="/privacy-policy">Privacy Policy</Link><Link href="/cookie-policy">Cookie Policy</Link><Link href="/trasparenza">Trasparenza</Link></div>
    <div className="footer-column footer-social"><h2>Seguici</h2><div className="social-links">{socialLinks.map(link=><a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`Visita il profilo ${link.label} della Nazionale Italiana Sanitari (si apre in una nuova scheda)`}><SocialIcon name={link.label}/><span>{link.label}</span></a>)}</div></div>
  </div>
