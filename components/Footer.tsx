...
export default function Footer(){return <footer className="footer">
  <div className="shell footer-grid">
    ...
    <div className="footer-column footer-social"><h2>Seguici</h2><div className="social-links">{socialLinks.map(link=><a key={link.label} href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`${link.label} NIS (si apre in una nuova scheda)`}>{link.label}</a>)}</div></div>
  </div>
